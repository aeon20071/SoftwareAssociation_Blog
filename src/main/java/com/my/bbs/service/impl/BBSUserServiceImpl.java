...
public class BBSUserServiceImpl implements BBSUserService {
    ...
    public Admin getAdmin(String aname) {
        return bbsUserMapper.getAdmin(aname);
    }

    @Override
    public List<BBSUser> getAllUser() {
        return bbsUserMapper.getAllUser();
    }

    @Override
    public int changeUserState(Long userId) {
        BBSUser bbsUser = bbsUserMapper.selectByPrimaryKey(userId);
        Byte userStatus = bbsUser.getUserStatus();
        int i = userStatus.intValue();
        int pojo;
        if (i==0){
            pojo=1;
        }else {
        ...
        bbsUserMapper.changeUserState(userId,pojo);
        return 0;
    }

    @Override
    public int delUser(Long userId) {
        return bbsUserMapper.delUser(userId);
    }

    @Override
    public List<BBSUser> getUserBySearch(String value) {
        return bbsUserMapper.getUserBySearch(value);
    }
}
